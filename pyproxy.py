...
def requests_handler(buffer):
    # perform packet modification (modify this to customize proxy - Fuzzing, authentications, finding creds, etc)
    return buffer

def response_handler(buffer):
    #perform packet mod (modify this to customize proxy - Fuzzing, authentications, finding creds, etc)
    return buffer
...
def main():
    if len(sys.argv[1:]) !=5:
        print("Usage: python3 pyproxy.py [localhost] [localport]", end='')
        print("[remotehost] [remoteport] [recieve_first]")
        print("Example: python3 pyproxy.py 127.0.0.1 9000 10.12.132.1 9000 True")
        sys.exit()
